...
const numCls =
  'w-20 px-2 py-1 text-sm rounded-xl border outline-none focus:border-opacity-80';

function MemberCard({ m, t, highlight = false }: any) {
  const u = m?.user || {};
  const name = u.displayName || u.username || '';
  return (
    <Link href={`/players/${m.userId}`} className={`flex items-center gap-3 rounded-xl border p-3 shadow-sm transition-all duration-200 ${highlight ? '' : ''}`} style={{ background: 'var(--card-bg)', borderColor: highlight ? 'var(--badge-warning-text)' : 'var(--card-border)' }}>
      {u.avatar ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={avatarSrc(u.avatar, 96)} alt={name} referrerPolicy="no-referrer" className="h-12 w-12 rounded-xl border object-cover" style={{ borderColor: 'var(--card-border)' }} />
      ) : (
        <div className="flex h-12 w-12 items-center justify-center rounded-xl font-bold text-lg" style={{ background: 'var(--accent-primary)', color: 'var(--badge-success-text)' }}>{name?.[0]?.toUpperCase() || 'J'}</div>
      )}
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          {m.isCaptain && <Crown size={14} className="shrink-0" style={{ color: 'var(--badge-warning-text)' }} />}
          <p className="truncate text-sm font-medium" style={{ color: 'var(--page-text)' }}>{name}</p>
        </div>
        <div className="mt-1 flex flex-wrap items-center gap-1.5">
          {m.role && <Badge variant="purple" size="sm" className="gap-1"><RoleIcon role={m.role} size={14} /> {t('lane.' + m.role)}</Badge>}
          {hasRankBadge(u.gameRank) && <span className="inline-flex items-center gap-1 text-xs" style={{ color: 'var(--sidebar-text)' }}><RankBadge rank={u.gameRank} size={16} /> {u.gameRank}</span>}
        </div>
      </div>
    </Link>
  );
...
function MatchRow({ m, t, onResult }: any) {
  ...
  return (
    <div className="rounded-xl border p-3 shadow-sm" style={{ background: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 text-sm">
        <span className={`truncate text-right ${aWin ? 'font-bold' : ''}`} style={{ color: aWin ? 'var(--badge-warning-text)' : 'var(--page-text)' }}>{m.teamA?.name}</span>
        <span className="shrink-0 px-2 font-semibold" style={{ color: 'var(--sidebar-text)' }}>{completed ? `${m.scoreA} - ${m.scoreB}` : 'vs'}</span>
        <span className={`truncate text-left ${bWin ? 'font-bold' : ''}`} style={{ color: bWin ? 'var(--badge-warning-text)' : 'var(--page-text)' }}>{m.teamB?.name}</span>
      </div>
      <div className="mt-2 flex flex-wrap items-center justify-center gap-2">
        ...
        {dateLabel && <span className="text-xs" style={{ color: 'var(--sidebar-text)' }}>{dateLabel}</span>}
...
function ApplicationRow({ a, t, onDecide, onContact, acting }: any) {
  ...
  const name = u.displayName || u.username || '—';
  return (
    <div className="flex items-center gap-3 rounded-xl border p-2.5 shadow-sm" style={{ background: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
      {u.avatar ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={avatarSrc(u.avatar, 64)} alt={name} referrerPolicy="no-referrer" className="h-8 w-8 shrink-0 rounded-full object-cover" />
      ) : (
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full font-bold text-xs" style={{ background: 'var(--accent-primary)', color: 'var(--badge-success-text)' }}>{name[0]?.toUpperCase() || 'J'}</div>
      )}
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <Link href={`/players/${a.userId}`} className="truncate text-sm font-medium hover:opacity-80 transition-opacity" style={{ color: 'var(--page-text)' }}>{name}</Link>
          {a.role && <Badge variant="purple" size="sm" className="gap-1"><RoleIcon role={a.role} size={13} /> {t('lane.' + a.role)}</Badge>}
        </div>
        {a.message && <p className="truncate text-xs" style={{ color: 'var(--sidebar-text)' }}>{a.message}</p>}
      </div>
  ...
  if (!team) {
    return (
      <div className="mx-auto max-w-4xl space-y-6">
        <Link href="/teams" className="inline-flex items-center gap-1.5 text-sm hover:opacity-80 transition-opacity" style={{ color: 'var(--accent-primary)' }}><ArrowLeft size={16} /> {t('teams.back')}</Link>
      ...
      label: (
        <span className="flex items-center gap-1.5">
          ...
          {canManage && pendingCandidates > 0 && (
            <span className="rounded-full px-1.5 text-[10px]" style={{ background: 'rgba(var(--accent-primary-rgb, 59, 130, 246), 0.1)', color: 'var(--accent-primary)' }}>{pendingCandidates}</span>
          )}
        </span>
      ),
    },
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <Card className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-5">
        <div className="h-24 w-24 shrink-0 overflow-hidden rounded-full border-2 sm:h-28 sm:w-28" style={{ borderColor: 'var(--card-border)', background: 'var(--surface-bg)' }}>
          {team.image ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={team.image} alt={team.name} referrerPolicy="no-referrer" className="w-full h-full object-cover" />
          ) : (
            <div className="flex h-full w-full items-center justify-center text-3xl font-bold" style={{ background: 'var(--accent-primary)', color: 'var(--badge-success-text)' }}>{team.name?.[0]?.toUpperCase() || 'T'}</div>
          )}
        </div>
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="text-2xl font-bold sm:text-3xl" style={{ color: 'var(--page-text)' }}>{team.name}</h1>
            <Badge variant={team.type === 'esport' ? 'gold' : 'default'} size="sm">{t('admin.esport.badge.' + (team.type || 'community'))}</Badge>
          </div>
          <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm" style={{ color: 'var(--sidebar-text)' }}>
            <span className="inline-flex items-center gap-1.5"><Users size={14} style={{ color: 'var(--accent-primary)' }} />{team.memberCount ?? members.length} {t('teams.members')}</span>
            {foundedLabel && <span className="inline-flex items-center gap-1.5"><Calendar size={14} style={{ color: 'var(--accent-primary)' }} />{t('teams.detail.founded')} {foundedLabel}</span>}
          </div>
          {team.description && <p className="mt-2 whitespace-pre-line text-sm" style={{ color: 'var(--sidebar-text)' }}>{team.description}</p>}
        </div>
      </Card>
        ...
        <div>
          {amCaptain ? (
            members.length === 0 ? <EmptyState icon={<Users size={28} />} title={t('teams.detail.noMembers')} /> : (
              <div className="space-y-2">
                {members.map((m) => {
                  ...
                  return (
                    <div key={m.id ?? m.userId} className="flex flex-col gap-2 rounded-xl border p-2.5 shadow-sm sm:flex-row sm:items-center" style={{ background: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
                      <div className="min-w-0 flex-1 flex items-center gap-2">
                        <span className="truncate text-sm" style={{ color: 'var(--page-text)' }}>{u.displayName || u.username}</span>
            ...
            <div className="space-y-5">
              {captainMember && (
                <div>
                  <div className="flex items-center gap-2 mb-2.5"><Crown size={15} style={{ color: 'var(--badge-warning-text)' }} /><span className="text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--badge-warning-text)' }}>{t('teams.detail.captain')}</span></div>
                  <MemberCard m={captainMember} t={t} highlight />
                </div>
              )}
              {starters.length > 0 && (
                <div>
                  <h3 className="mb-2.5 text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--sidebar-text)' }}>{t('teams.detail.starters')}</h3>
              ...
              {substitutes.length > 0 && (
                <div>
                  <h3 className="mb-2.5 text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--sidebar-text)' }}>{t('teams.detail.substitutes')}</h3>
        ...
        <div className="space-y-4">
          ...
          {campaigns.length === 0 ? (
            ...
            campaigns.map((c) => {
              ...
              return (
                <div key={c.id} className="rounded-xl border p-4 shadow-sm" style={{ background: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
                  <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                    ...
                    {canManage ? (
                      ...
                      applied ? <span className="text-xs" style={{ color: 'var(--sidebar-text)' }}>{t('recruitment.applied')}</span> : <Button size="sm" onClick={() => openApply(c)}><Send size={14} /> {t('recruitment.apply')}</Button>
                    ) : null}
                  </div>
                  {c.message && <p className="mb-3 whitespace-pre-line text-sm" style={{ color: 'var(--sidebar-text)' }}>{c.message}</p>}

                  {canManage && (
                    <div className="border-t pt-3" style={{ borderColor: 'var(--card-border)' }}>
                      <p className="mb-2 text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--sidebar-text)' }}>{t('recruitment.candidates')}{c.applicationCount ? ` (${c.applicationCount})` : ''}</p>
                      {(c.applications || []).length === 0 ? (
                        <p className="text-sm" style={{ color: 'var(--sidebar-text)' }}>{t('recruitment.noCandidates')}</p>
                      ) : (
                        <div className="space-y-2">
      ...
      <Modal open={newOpen} onClose={() => setNewOpen(false)} closeLabel={t('common.close')} title={t('recruitment.newTitle')} icon={<Megaphone size={20} />} headerVariant="gradient">
        <form onSubmit={createCampaign} className="space-y-4">
          <div>
            <label className="mb-1 block text-sm font-medium" style={{ color: 'var(--page-text)' }}>{t('recruitment.pickRoles')}</label>
            <p className="mb-2 text-xs" style={{ color: 'var(--sidebar-text)' }}>{t('recruitment.pickRolesHint')}</p>
            <div className="space-y-2">
              {LANES.map((l) => (
                <div key={l} className="flex items-center gap-2">
                  <RoleIcon role={l} size={16} />
                  <span className="flex-1 text-sm" style={{ color: 'var(--sidebar-text)' }}>{t('lane.' + l)}</span>
      ...
      <Modal open={!!applyCampaign} onClose={() => setApplyCampaign(null)} closeLabel={t('common.close')} title={t('recruitment.applyTitle')} icon={<Send size={20} />} headerVariant="gradient">
        <form onSubmit={submitApply} className="space-y-4">
          <div className="space-y-1.5">
            <label className="block text-sm font-medium" style={{ color: 'var(--page-text)' }}>{t('recruitment.applyRole')}</label>
      ...
      <Modal open={!!resultMatch} onClose={() => setResultMatch(null)} closeLabel={t('common.close')} title={t('admin.matches.result')} icon={<Check size={20} />} headerVariant="gradient">
        {resultMatch && (
          <form onSubmit={submitResult} className="space-y-4">
            <p className="text-center text-sm" style={{ color: 'var(--page-text)' }}>{resultMatch.teamA?.name} <span style={{ color: 'var(--sidebar-text)' }}>vs</span> {resultMatch.teamB?.name}</p>
